...
複数ファイルをドラッグしての連続処理にも対応しています。変換前に一時停止して、生成ファイルを編集してから続行することも可能です。

初回に「よく使う設定」にしておけば、あとは動画ファイルをドラッグするだけでガンガン自動処理してくれて便利なんだね、と考えてください。

同様のGUIツールもありますが、毎日繰り返し行う作業で、ボタンをポチポチクリックしまくる必要があるGUIは好きになれません。「設定はファイル冒頭のフラグだけ」「ファイルは1つだけ」に拘りました。操作はドラッグのみです。

### 実行に必要なツール

FavsEの動作に必要なツールは以下の通りで、結構あります。設定内容によっては無くても構わないものがありますが、今後の更新でどうなるかわかりませんので一応すべて入れておくのが無難です。

なお、以後すべて**32bit(x86版)**で揃えてください。AviSynth+や64bit(x64版)で揃えていた時は色々面倒で凝りました。
