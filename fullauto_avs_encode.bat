...
REM ----------------------------------------------------------------------
if %use_nvenvc% == 1 (
  set nvencc_opt=--avs --qp-init 20:23:25 --qp-min 19:22:24 --vbr-quality 0 --lookahead 32 --ref 5 --weightp --aq --aq-temporal --aq-strength 0 --direct auto
) else (
  set x264_opt=--preset slower --crf 21 --partitions p8x8,b8x8,i8x8,i4x4 --ref 5 --subme 8 --no-fast-pskip --no-dct-decimate
)
...
:no_del_temp
echo すべての一時ファイルは残っており、次回エンコードする際に再利用できます。
echo 処理をやり直したい部分の一時ファイルを削除すると、次回処理時には該当処理をスキップします。
